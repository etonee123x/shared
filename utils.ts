...
export const createError = ({ data, statusCode }: Partial<Omit<CustomError, '__isCustomError'>>) =>
  ({ data, statusCode: isNil(statusCode) ? CUSTOM_ERROR_UNKNOWN_STATUS_CODE : statusCode, __isCustomError: true });

export const createErrorClient = (data: CustomError['data']) => createError({
  data,
  statusCode: 400,
});

export const createErrorServer = (data: CustomError['data']) => createError({
  data,
  statusCode: 500,
});
